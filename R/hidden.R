...
check.offset <- function(args) {
  if(sum(names(args)=="offset")==1) {
    cat("Error: Offsets are currently not supported in twoe.\n")
...
check.interval.hmodels <- function (interval,nobs) {
...
check.area.hmodels <- function (area,nobs) {
  if (sum(!is.numeric(area))!=0) {
    cat("Error: area must be a numeric scalar or vector of length equal to the number of observations.\n")
    stop("Please respecify and call ", calling.function(), " again.",
         call.=FALSE)
  }
  if (length(area)==1) {
    return(rep(area,nobs))
  }
  if (length(area)>1 & length(area)!=nobs) {
    cat("Error: area must be a numeric scalar or vector of length equal to the number of observations.\n")
    stop("Please respecify and call ", calling.function(), " again.",
         call.=FALSE)
  }
...
check.diameter.hmodels <- function (diameter,nobs) {
  if (sum(!is.numeric(diameter))!=0) {
    cat("Error: diameter must be a numeric vector of length equal to the number of observations.\n")
    stop("Please respecify and call ", calling.function(), " again.",
         call.=FALSE)
  }
  if (length(diameter)!=nobs) {
    cat("Error: diameter must be a numeric vector of length equal to the number of observations.\n")
    stop("Please respecify and call ", calling.function(), " again.",
         call.=FALSE)
  }
...
check.positive <- function (x) {
  if (x<=0) {
    cat("Error: sd.a and sd.b must be strictly positive.\n")
    stop("Please respecify and call ", calling.function(), " again.",
         call.=FALSE)
  }
  if (x>0) {
