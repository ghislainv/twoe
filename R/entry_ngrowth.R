entry_ngrowth <- function (data_growth,a.sd=0.927,b.sd=0.0038,burnin=1000,mcmc=1000,thin=1,th.sign=5) {
  ...
  model <- entry_ngrowth_gibbs(fixed=G~lD+lC,
                               random=~lD+lC,
                               group="Sp",
                               diameter=D,
                               data=DataGrowth,
  ...
  pdf(file="posteriors_ngrowth.pdf")
  plot(mcmc.obj)
  dev.off()
  
  ########################
  # Export MCMCs
  write.table(MCMC,file="MCMC_ngrowth.txt",sep="\t",row.names=FALSE)
  ...
  # D
  pdf("ngrowth_dbh.pdf", pointsize=8, onefile=TRUE, family="Helvetica", paper="letter")
  ...
  for (k in 1:nspecies) {
    ...
    G.hat <- vector()
    for (n in 1:100) {
      G.hat[n] <- exp(b0sp+b1sp*log(D.seq[n])+b2sp*log(C.fixed+1))
    }
    ...
    G.hat.msp <- vector()
    for (n in 1:100) {
      G.hat.msp[n] <- exp(b0.msp+b1.msp*log(D.seq[n])+b2.msp*log(C.fixed+1))
    }
  ...
  # C
  pdf("ngrowth_comp.pdf", pointsize=8, onefile=TRUE, family="Helvetica", paper="letter")
  par(mfrow=c(4,3),cex=1, las=1, lwd=1, pch=18, cex.lab=1, mar=c(4,4,2,1))
  ...
  for (k in 1:nspecies) {
    ...
    G.hat <- vector()
    for (n in 1:100) {
      G.hat[n] <- exp(b0sp+b1sp*log(D.fixed)+b2sp*log(C.seq[n]+1))
    }
    ymin.Pred <- min(G.hat); ymax.Pred <- max(G.hat)
    ...
    G.hat.msp <- vector()
    for (n in 1:100) {
      G.hat.msp[n] <- exp(b0.msp+b1.msp*log(D.fixed)+b2.msp*log(C.seq[n]+1))
    }
    ymin.Pred.msp <- min(G.hat.msp); ymax.Pred.msp <- max(G.hat.msp)
    ymin.Graph <- min(ymin.Data,ymin.MLL,ymin.Pred,ymin.Pred.msp)
    ymax.Graph <- max(ymax.Data,ymax.MLL,ymax.Pred,ymax.Pred.msp)
  ...
  write.table(sppar_growth,file="sppar_ngrowth.txt",sep="\t",row.names=FALSE)
  
cat("\nFunction entry_ngrowth has finished, please check files:
1. time_ngrowth.txt
2. posteriors_ngrowth.pdf
3. MCMC_ngrowth.txt
4. ngrowth_dbh.pdf
5. ngrowth_comp.pdf
6. sppar_ngrowth.txt
