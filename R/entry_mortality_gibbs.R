#===================================================================
# entry_mortality_gibbs.R
#
# entry_mortality_gibbs() samples from the posterior distribution of a
# Binomial linear regression model in R using linked C++
# code in Scythe
#
...
entry_mortality_gibbs <- function (fixed, random, group, interval=1, data, burnin=5000,
                                   mcmc=10000, thin=10, 
...

{
  ...
  if (is.null(random)) {
    ...
    logit_theta_pred <- rep(logit(0.5),nobs)
    Deviance <- rep(0,nsamp)

    #========
    # call C++ code to draw sample
    #========
    Sample <- .C("entry_mortality_gibbs_fixed",
                 #= Constants and data
                 ...
                 logit_theta_pred.nonconst=as.double(logit_theta_pred), ## Predictive posterior mean
                 #= Seeds
    ...
    Matrix[,ncol(Matrix)-1] <- Sample[[10]]
  ...
  if (!is.null(random)) {
    ...
    # call C++ code to draw sample
    #========
    Sample <- .C("entry_mortality_gibbs_mixed",
                 #= Constants and data
                 ngibbs=as.integer(ngibbs), nthin=as.integer(nthin), nburn=as.integer(nburn), ## Number of iterations, burning and samples
    ...
    #= Filling-in the matrix
    Matrix[,c(1:np)] <- matrix(Sample[[13]],ncol=np)
    Matrix[,c((np+1):(np+nq*ngroup))] <- matrix(Sample[[14]],ncol=nq*ngroup)
    Matrix[,c((np+nq*ngroup+1):(np+nq*ngroup+nq*nq))] <- matrix(Sample[[15]],ncol=nq*nq)
