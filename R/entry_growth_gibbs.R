#===================================================================
# entry_growth_gibbs.R
#
# entry_growth_gibbs() samples from the posterior distribution of a
...
entry_growth_gibbs <- function (fixed, random, group, data, burnin=1000,
                                mcmc=10000, thin=10, 
                                ...
                                nu=0.001, delta=0.001, ...)
{
  ...
  if (is.null(random)) {
    ...
    #========
    Sample <- .C("entry_growth_gibbs_fixed",
                 #= Constants and data
    ...
    logL.hat <- sum(dnorm(Y,X%*%beta.mean,sd=sqrt(V.mean),1)) # log Likelihood
    D.hat <- -2*logL.hat
    ...
    #= Output
    return (list(mcmc=MCMC,Y.pred=Sample[[15]],pD=pD,DIC=DIC))
  }
  ...
  if (!is.null(random)) {
    ...
    # call C++ code to draw sample
    #========
    Sample <- .C("entry_growth_gibbs_mixed",
                 #= Constants and data
                 ngibbs=as.integer(ngibbs), nthin=as.integer(nthin), nburn=as.integer(nburn),## Number of iterations, burning and samples
    ...
    logL.hat <- 0
    for (i in 1:nobs) {
      logL.hat <- logL.hat+dnorm(Y[i],
                                 X[i,]%*%beta.mean+W[i,]%*%bk.mean[,c(IdentGroup[i]+1)],
                                 sd=sqrt(V.mean),1) # log Likelihood
    }
    D.hat <- -2*logL.hat
    D.bar <- Matrix.mean[ncol(Matrix)]
    pD <- as.numeric(D.bar-D.hat)
    ...
    #= Output
    return (list(mcmc=MCMC,Y.pred=Sample[[23]],pD=pD,DIC=DIC))
  }
}
