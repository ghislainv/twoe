////////////////////////////////////////////////////////////////////
// entry_recruitment_gibbs_mixed.cc
//
// entry_recruitment_gibbs_mixed.cc samples from the posterior distribution of a
// Poisson hierarchical linear regression model
//
// The code uses Algorithm 2 of Chib & Carlin (1999) for efficient
// inference of (\beta | log(lambda), sigma^2, Vb).
//
...
extern "C"{
    ...
    void entry_recruitment_gibbs_mixed (
	...
	for (int g=0;g<NGIBBS;g++) {
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	...
	    	for (int m=0; m<nobsk[k]; m++) {
	    	    ...
	    	    // lambda_prim
	    	    double lambda_prim_prop=exp(log_lambda_prop)*Int_vect[w]*SCell_vect[w];
	    	    double lambda_prim_run=exp(log_lambdak_run[k](m))*Int_vect[w]*SCell_vect[w];

	    	    // Ratio of probabilities
	    	    double p_prop=log(dpois(Y_vect[w],lambda_prim_prop))+
	    		log(dnorm(log_lambda_prop,log_lambda_hat(0),sqrt(V_run)));

	    	    double p_now=log(dpois(Y_vect[w],lambda_prim_run))+
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	for (int m=0; m<nobsk[k]; m++) {
	    	    ...
	    	    double lambda_prim_run=exp(log_lambdak_run[k](m))*Int_vect[w]*SCell_vect[w];
	    	    // L
	    	    logLk+=log(dpois(Y_vect[w],lambda_prim_run));
	    	}
	    }
    ...
    } //  end entry_recruitment_gibbs_mixed function
