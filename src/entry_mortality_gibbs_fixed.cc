////////////////////////////////////////////////////////////////////
// entry_mortality_gibbs_fixed.cc
//
// entry_mortality_gibbs_fixed.cc samples from the posterior distribution of a
// Binomial linear regression model
//
////////////////////////////////////////////////////////////////////
//
...
extern "C"{
    ...
    void entry_mortality_gibbs_fixed (
	...
	double V_run=V[0];
	Matrix<double> *logit_thetai_run = new Matrix<double>[NOBS];
	for (int i=0;i<NOBS;i++) {
	    logit_thetai_run[i] = Matrix<double>(1,1);
	    logit_thetai_run[i](0)=logit_theta_pred[i];
	    logit_theta_pred[i]=0; // We reinitialize theta_pred to zero to compute mean of MCMC
	}
	...
	for (int g=0;g<NGIBBS;g++) {
	    ...
	    for (int i=0; i<NOBS; i++) {
	    	e+=crossprod(logit_thetai_run[i]-Xi_arr[i]*beta_run);
	    }
