////////////////////////////////////////////////////////////////////
// entry_mortality_gibbs_mixed.cc
//
// entry_mortality_gibbs_mixed.cc samples from the posterior distribution of a
// Binomial hierarchical linear regression model
//
...
extern "C"{
    ...
    void entry_mortality_gibbs_mixed (
	...
	for (int k=0;k<NGROUP;k++) {
	    ...
	    for (int m=0; m<nobsk[k]; m++) {
		logit_thetak_run[k](m)=logit_theta_pred[posk_arr[k][m]];
		logit_theta_pred[posk_arr[k][m]]=0; // We reinitialize theta_pred to zero to compute the posterior mean
	    }
	}
	double Deviance_run=Deviance[0];

        ////////////////////////////////////////////////////////////
	// Proposal variance and acceptance for adpative sampling //
	...
	for (int g=0;g<NGIBBS;g++) {
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	...
	    	for (int m=0; m<nobsk[k]; m++) {
	    	    ...
	    	    // theta_prim
	    	    double theta_prim_prop=1-pow(1-invlogit(logit_theta_prop),Int_vect[w]);
	    	    double theta_prim_run=1-pow(1-invlogit(logit_thetak_run[k](m)),Int_vect[w]);

	    	    // Ratio of probabilities
	    	    double p_prop=log(dbinom(Y_vect[w],1,theta_prim_prop))+
	    		log(dnorm(logit_theta_prop,logit_theta_hat(0),sqrt(V_run)));

	    	    double p_now=log(dbinom(Y_vect[w],1,theta_prim_run))+
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	for (int m=0; m<nobsk[k]; m++) {
	    	    ...
	    	    double theta_prim_run=1-pow(1-invlogit(logit_thetak_run[k](m)),Int_vect[w]);
	    	    // L
	    	    logLk+=log(dbinom(Y_vect[w],1,theta_prim_run));
	    	}
	    }
    ...
    } //  end entry_mortality_gibbs_mixed function
