////////////////////////////////////////////////////////////////////
// entry_ngrowth_gibbs_mixed.cc
//
// entry_ngrowth_gibbs_mixed.cc samples from the posterior distribution of a
...
// inference of (\beta | log(Y), sigma^2, Vb).
//
...
extern "C"{
    ...
    void entry_ngrowth_gibbs_mixed (
	...
	double V_run=V[0];
	Matrix<double> *log_Yk_run = new Matrix<double>[NGROUP];
	for (int k=0;k<NGROUP;k++) {
	    log_Yk_run[k] = Matrix<double>(nobsk[k],1);
	    for (int m=0; m<nobsk[k]; m++) {
		log_Yk_run[k](m)=log(Y_true[posk_arr[k][m]]);
		Y_true[posk_arr[k][m]]=0; // We reinitialize Y_true to zero to compute mean of MCMC
	    }
	}
	...
	for (int g=0;g<NGIBBS;g++) {
            ...
            ///////////////////////////////////////////////////
            // vector log_Y :: Metropolis-Hastings algorithm // 
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	...
	    	for (int m=0; m<nobsk[k]; m++) {
	    	    ...
	    	    int w=posk_arr[k][m];
                    ...
                    // Mean of the prior
	    	    Matrix<double> log_Y_hat=Xk_arr[k](m,_)*beta_run+Wk_arr[k](m,_)*bk_run[k];

	    	    // Proposal
	    	    double log_Y_prop=myrng.rnorm(log_Yk_run[k](m),sigmap[w]);
		
	    	    // Y
	    	    double Y_prop=exp(log_Y_prop);
	    	    double Y_run=exp(log_Yk_run[k](m));

	    	    // Ratio of probabilities
	    	    double p_prop=log(dnorm(Y_vect[w],Y_prop,a_sd[0]+b_sd[0]*D_vect[w]))+
	    		log(dnorm(log_Y_prop,log_Y_hat(0),sqrt(V_run)));

	    	    double p_now=log(dnorm(Y_vect[w],Y_run,a_sd[0]+b_sd[0]*D_vect[w]))+
	    		log(dnorm(log_Yk_run[k](m),log_Y_hat(0),sqrt(V_run)));
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	...
	    	sum_v += (1/V_run)*tXk_arr[k]*log_Yk_run[k]-pow(1/V_run,2)*tXWk_arr[k]*invpd(invpd(Vb_run)+tWk_arr[k]*(1/V_run)*Wk_arr[k])*tWk_arr[k]*log_Yk_run[k];
	    }
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	...
	    	// small_vk
	    	Matrix<double> small_vk=(t(Wk_arr[k])*(log_Yk_run[k]-Xk_arr[k]*beta_run))/V_run;
	    ...
	    for (int k=0; k<NGROUP; k++) {
	    	e+=crossprod(log_Yk_run[k]-Xk_arr[k]*beta_run-Wk_arr[k]*bk_run[k]);
	    }
	    ...
	    for (int k=0; k<NGROUP; k++) {
		    ...
		    double Y_run=exp(log_Yk_run[k](m));
	    	    // L
	    	    logLk+=log(dnorm(Y_vect[w],Y_run,a_sd[0]+b_sd[0]*D_vect[w]))+log(dnorm(log_Yk_run[k](m),log_Y_hat(0),sqrt(V_run)));
	    	}
	    }
		...
		int isamp=((g+1)-NBURN)/(NTHIN);
	    	...
	    	for (int k=0;k<NGROUP;k++){
	    	    for (int m=0; m<nobsk[k]; m++) {
	    		int w=posk_arr[k][m];
			Y_true[w]+=exp(log_Yk_run[k](m))/NSAMP; // We compute the mean of NSAMP values
			Matrix<double> log_Y_hat=Xk_arr[k](m,_)*beta_run+Wk_arr[k](m,_)*bk_run[k];
	    		log_Y_pred[w]+=log_Y_hat(0)/NSAMP; // We compute the mean of NSAMP values
	    	    }
	    	}
	    }
    ...
    } //  end entry_ngrowth_gibbs_mixed function
